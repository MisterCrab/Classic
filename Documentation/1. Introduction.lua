...
* [6] is Passive rotation (limited actions, usually @player, @party1, @raid1, @arena1)
...
Second group must be named "Shown Cast Bars" and contain max 3 icons per 1 row, this means what you can have totally 6 icons with 2 rows, type of group must be "Bar" (horizontal) and follow right after "Shown Main". It does use "Flat" texture of casting bar, make sure what you have selected it in /tmw > 'General' > 'Main Options' (or 'Main Settings') 
[1]		[2]		[3]
[4]		[5]		[6]
@arena1 @arena2 @arena3
This is used to interrupt by Kick Arena1-3 and option timings
* [1] -> [3] is green (Heals) / red (PvP) PlayerKick / yellow AutCastBarsSpells (such as Ground Totem)
...
After "Shown Cast Bars" group to the right of it follows HealingEngine frame, so if you make 4+ icons in 1 row of "Shown Cast Bars" group it will has position higher than HealingEngine which will make you conflict on healers and for rank spells 

For Classic added additional rank frames below HealingEngine frame, where first coming is for single and last coming is for secondary rotation 
...
Hard to figure? 
Let's make it easier, I prepared template profile named '[GGL] Basic', you can create new profile with own name and then use 'Copy' from '[GGL] Basic' which will make for you prepared for 'The Action' API these 2 groups. So when that is done you can build own routine now.
]]
